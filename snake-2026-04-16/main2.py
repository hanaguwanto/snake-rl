...
class Player:
    ...
    def generate_random_position(self):
        return (random.randint(4, GRID_SIZE - 4) * CELL_SIZE, random.randint(4, GRID_SIZE - 4) * CELL_SIZE)
    ...
    def nabrak(self, lawan):
        ...
        if x < 0 or x >= WINDOW_SIZE or y < 0 or y >= WINDOW_SIZE:
            return True
        if self.head in list(self.snake)[1:] or self.head in list(lawan.snake)[1:]:
            return True
        if lawan.head == self.head:
            return True
        return False
...
def get_state(player, apel, lawan):
    ...
    player_left = player.head[0] < lawan.head[0]
    player_right = player.head[0] > lawan.head[0]
    player_up = player.head[1] < lawan.head[1]
    player_down = player.head[1] > lawan.head[1]
...
def show_countdown(screen, clock):
    ...
    for i in ["3", "2", "1"]:
        ...
        pygame.time.delay(1000)  # 1 detik
        clock.tick(60)
...
def main():
    ...
    bg = pygame.image.load("graphics/grass.png")
    ...
    while running:
        ...
        # ================= GAME LOGIC =================
        if started and current_time - last_move_time >= MOVE_DELAY:
            ...
            # AI MOVE
            state = get_state(ai, apel_pos, player)
            with torch.no_grad():
        ...
        # tampilkan instruksi kalau belum mulai
        if not started:
            font = pygame.font.Font(None, 40)
            text = font.render("PRESS ARROW / WASD", True, (255, 255, 255))
            rect = text.get_rect(center=(WINDOW_SIZE // 2, WINDOW_SIZE // 2))
            screen.blit(text, rect)
